...
//     empty AND INTELLIRECON_LLM matches Legacy_Provider_Shape;
...
// the configured INTELLIRECON_LLM value, returning the bare model name.
...
//  2. Catalog empty AND INTELLIRECON_LLM matches Legacy_Provider_Shape
...
func (s *Server) legacyOrCatalogDefaultEndpoint(ctx context.Context, cfg *config.Config) llm.Endpoint {
	...
	// (internal/llm/resolver.go): when INTELLIRECON_LLM_PROFILE names a
	...
	if cfg != nil && strings.TrimSpace(cfg.LLMProfile) != "" && s.catalog != nil && s.profiles != nil {
		...
		if prof, ok, err := s.profiles.Get(ctx, key); err == nil && ok {
			if entry, ok, err := s.catalog.Get(ctx, prof.Provider); err == nil && ok {
				...
				// default so INTELLIRECON_LLM=google/gemini-test-model
	...
	// INTELLIRECON_LLM names one of the eight known legacy slugs;
