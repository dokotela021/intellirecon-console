...
func TestMain(m *testing.M) {
	os.Setenv("INTELLIRECON_OOB_PUBLIC_URL", "http://oob.test.example")
	os.Setenv("INTELLIRECON_OOB_PORT", "0") // 0 → ensureStarted errors, but Generate registers before that only after start; see note
	os.Exit(m.Run())
}
...
func TestEnabledAndPublicBaseURL(t *testing.T) {
	if !Enabled() {
		t.Fatal("OOB should be enabled when INTELLIRECON_OOB_PUBLIC_URL is set")
	}
...
func TestHandleRecordsRegisteredToken(t *testing.T) {
	...
	if !strings.Contains(rr.Body.String(), "intellirecon-oob-ok:"+tok) {
