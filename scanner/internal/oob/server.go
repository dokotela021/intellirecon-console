...
//
// This is essential under IntelliRecon's "no theoretical findings" policy: a blind
...
// listener binds 0.0.0.0:<INTELLIRECON_OOB_PORT>; the operator exposes it publicly
// (directly or via reverse proxy) and sets INTELLIRECON_OOB_PUBLIC_URL to the
...
// listener (INTELLIRECON_OOB_PUBLIC_URL). When set it takes precedence over the
...
func Generate() (callbackURL, token string, err error) {
	...
	if !interactshEnabled() {
		return "", "", fmt.Errorf("OOB is disabled (INTELLIRECON_OOB_DISABLE=true)")
	}
...
func ensureStarted() error {
	startOnce.Do(func() {
		...
		if port <= 0 {
			startErr = fmt.Errorf("INTELLIRECON_OOB_PORT not set")
			return
		}
...
func selfHostedGenerate() (callbackURL, token string, err error) {
	if !selfHosted() {
		return "", "", fmt.Errorf("OOB is not configured (set INTELLIRECON_OOB_PUBLIC_URL and INTELLIRECON_OOB_PORT)")
	}
...
func handle(w http.ResponseWriter, r *http.Request) {
	...
	_, _ = w.Write([]byte("intellirecon-oob-ok:" + token + "\n"))
}
