...
func TestEnvHelpers(t *testing.T) {
	t.Setenv("INTELLIRECON_TEST_FLOAT", "")
	if got := envFloat("INTELLIRECON_TEST_FLOAT", 1.5); got != 1.5 {
		t.Fatalf("envFloat default = %v", got)
	}
	t.Setenv("INTELLIRECON_TEST_FLOAT", "2.25")
	if got := envFloat("INTELLIRECON_TEST_FLOAT", 1.5); got != 2.25 {
		t.Fatalf("envFloat parsed = %v", got)
	}
	t.Setenv("INTELLIRECON_TEST_FLOAT", "bad")
	if got := envFloat("INTELLIRECON_TEST_FLOAT", 1.5); got != 1.5 {
		t.Fatalf("envFloat invalid default = %v", got)
	}

	t.Setenv("INTELLIRECON_TEST_INT", "7")
	if got := envInt64("INTELLIRECON_TEST_INT", 3); got != 7 {
		t.Fatalf("envInt64 parsed = %v", got)
	}
	t.Setenv("INTELLIRECON_TEST_INT", "bad")
	if got := envInt64("INTELLIRECON_TEST_INT", 3); got != 3 {
		t.Fatalf("envInt64 invalid default = %v", got)
	}

	t.Setenv("INTELLIRECON_OPTIONAL_INT", "")
	if got := envOptionalInt("INTELLIRECON_OPTIONAL_INT"); got != 0 {
		t.Fatalf("envOptionalInt empty = %v", got)
	}
	t.Setenv("INTELLIRECON_OPTIONAL_INT", "4")
	if got := envOptionalInt("INTELLIRECON_OPTIONAL_INT"); got != 4 {
		t.Fatalf("envOptionalInt parsed = %v", got)
	}
	t.Setenv("INTELLIRECON_OPTIONAL_INT", "0")
	if got := envOptionalInt("INTELLIRECON_OPTIONAL_INT"); got != 0 {
