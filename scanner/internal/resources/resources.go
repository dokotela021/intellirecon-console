...
var (
	...
	cpuCautionPct  = envFloat("INTELLIRECON_CPU_CAUTION_PCT", 70)
	cpuCriticalPct = envFloat("INTELLIRECON_CPU_CRITICAL_PCT", 90)
	...
	diskCautionMB  = envInt64("INTELLIRECON_DISK_CAUTION_MB", 2048)  // 2 GB
	diskCriticalMB = envInt64("INTELLIRECON_DISK_CRITICAL_MB", 1024) // 1 GB
	...
	manualMaxInstances = envOptionalInt("INTELLIRECON_MAX_INSTANCES")
...
func init() {
	...
	// INTELLIRECON_SCAN_CPU_LOAD was a per-scan CPU budget input for the legacy
	...
	if _, ok := os.LookupEnv("INTELLIRECON_SCAN_CPU_LOAD"); ok {
		log.Printf("[RESOURCES] INTELLIRECON_SCAN_CPU_LOAD is deprecated and ignored; scan admission is now RAM-only. Use INTELLIRECON_HEAVY_TOOL_CPU_LOAD to tune per-tool CPU throttling.")
	}

	heavyToolCPULoad = envFloatDefault("INTELLIRECON_HEAVY_TOOL_CPU_LOAD", autoHeavyToolCPULoad(cores))
	scanOverheadMB = envInt64Default("INTELLIRECON_SCAN_OVERHEAD_MB", autoScanOverheadMB(totalMB))
	scanMemoryBudgetMB = envInt64Default("INTELLIRECON_SCAN_MEMORY_BUDGET_MB", autoScanMemoryBudgetMB(totalMB, cores))
	...
	ramCautionMB = envInt64("INTELLIRECON_RAM_CAUTION_MB", autoCaution)
	ramCriticalMB = envInt64("INTELLIRECON_RAM_CRITICAL_MB", autoCritical)

	if scanMemoryBudgetMB < 1024 {
		log.Printf("[RESOURCES] INTELLIRECON_SCAN_MEMORY_BUDGET_MB=%d too low, using 1024", scanMemoryBudgetMB)
		scanMemoryBudgetMB = 1024
	}
	if scanOverheadMB < 128 {
		log.Printf("[RESOURCES] INTELLIRECON_SCAN_OVERHEAD_MB=%d too low, using 128", scanOverheadMB)
		scanOverheadMB = 128
	}
	...
	if override, ok := envOptionalInt64("INTELLIRECON_HEAVY_TOOL_MEM_LIMIT_MB"); ok {
...
//   - Apply INTELLIRECON_MAX_INSTANCES only when explicitly configured.
...
func configureGoRuntimeLimits(totalMB int64) {
	override, hasOverride := envOptionalInt64("INTELLIRECON_GO_MEM_LIMIT_MB")
	if !hasOverride {
...
// ProtectCurrentProcess lowers the OOM-killer score for the intellirecon parent
...
func ProtectCurrentProcess() {
	...
	if err := os.WriteFile("/proc/self/oom_score_adj", []byte("-500"), 0644); err != nil { //nolint:gosec // G306: procfs ignores the file mode
		if !os.IsPermission(err) {
			log.Printf("[RESOURCES] Cannot protect intellirecon from OOM killer: %v", err)
		}
		return
	}
	log.Printf("[RESOURCES] intellirecon OOM score set to -500")
}
