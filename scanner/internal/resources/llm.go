...
//
// The cap is initialized lazily on first use from the INTELLIRECON_LLM_MAX_INFLIGHT
...
// initLLMSemaphore reads INTELLIRECON_LLM_MAX_INFLIGHT and sizes llmInFlight.
//
// Resolution order:
//  1. If INTELLIRECON_LLM_MAX_INFLIGHT parses to a positive integer, use it.
...
func initLLMSemaphore() {
	n, _ := strconv.Atoi(os.Getenv("INTELLIRECON_LLM_MAX_INFLIGHT"))
	if n <= 0 {
