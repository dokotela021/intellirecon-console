...
// signs in once with the well-tested CLI, and intellirecon reuses the result.
//
...
// INTELLIRECON_CODEX_AUTH_FILE overrides it (useful when the dashboard runs
...
func codexCredentialPath() string {
	if v := strings.TrimSpace(os.Getenv("INTELLIRECON_CODEX_AUTH_FILE")); v != "" {
		return v
	}
