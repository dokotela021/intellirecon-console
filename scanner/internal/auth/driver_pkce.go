...
//     6.1–6.5.
//
//  2. Paste fallback: when the operator has set INTELLIRECON_BIND to a
...
//
//   - loopback (INTELLIRECON_BIND unset / "127.0.0.1") → bind an
...
func (d *pkceDriver) callbackHandler(flow *pkceFlow) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write([]byte("Sign-in complete. You can close this tab and return to intellirecon."))
...
// intellirecon from a misbehaving upstream that streams unbounded
// data.
...
// (the dashboard's effective default when INTELLIRECON_BIND is
