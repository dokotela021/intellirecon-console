...
// TestClaude_ImportFromNestedClaudeAiOauth mirrors the credential
// shape written by current Claude CLI releases, where the token
// fields sit under a "claudeAiOauth" wrapper key with an
// epoch-millisecond expiresAt instead of the flat legacy layout.
// Regression coverage for the "claude cli credentials not found"
// report caused by the driver only understanding the flat shape.
func TestClaude_ImportFromNestedClaudeAiOauth(t *testing.T) {
	f := newClaudeTestFixture(t)

	body := `{
		"claudeAiOauth": {
			"accessToken": "nested-at",
			"refreshToken": "nested-rt",
	...
	if got.AccessToken != "nested-at" {
		t.Fatalf("AccessToken = %q, want %q", got.AccessToken, "nested-at")
	}
	if got.RefreshToken != "nested-rt" {
		t.Fatalf("RefreshToken = %q, want %q", got.RefreshToken, "nested-rt")
	}
	wantExp := time.UnixMilli(4102444800000).UTC()
	if !got.ExpiresAt.Equal(wantExp) {
		t.Fatalf("ExpiresAt = %v, want %v", got.ExpiresAt, wantExp)
	}
}
