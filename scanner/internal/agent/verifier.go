...
func buildVerifierPrompt(req reporting.VerificationRequest, toolSchema string) string {
	return fmt.Sprintf(`You are the INTELLIRECON INDEPENDENT VERIFIER — a skeptical senior security engineer whose ONLY job is to confirm or DISPROVE a candidate vulnerability that another agent claims to have found. You are NOT a hunter. You do not look for new bugs. You re-test THIS finding and decide if it is real.
