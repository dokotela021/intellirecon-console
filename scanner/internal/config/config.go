// Package config provides configuration management for IntelliRecon.
// All configuration is loaded from environment variables with INTELLIRECON_ prefix.
package config
...
// Config holds all IntelliRecon configuration.
type Config struct {
	// LLM settings
	LLM             string   // INTELLIRECON_LLM — model name (e.g. "openai/gpt-5.4", "anthropic/claude-sonnet-4-20250514")
	APIBase         string   // INTELLIRECON_API_BASE — API endpoint
	APIKey          string   // INTELLIRECON_API_KEY — API key
	LLMProfile      string   // INTELLIRECON_LLM_PROFILE — active credential pointer "<provider>:<profileId>" (v4.4.22+)
	ReasoningEffort string   // INTELLIRECON_REASONING_EFFORT — "low", "medium", "high"
	Temperature     *float64 // INTELLIRECON_TEMPERATURE — LLM temperature (0.0-2.0), default 0.2; pointer to distinguish unset from 0.0
	LLMMaxRetries   int      // INTELLIRECON_LLM_MAX_RETRIES
	MemCompTimeout  int      // INTELLIRECON_MEMORY_COMPRESSOR_TIMEOUT

	// Runtime settings
	RuntimeBackend string // INTELLIRECON_RUNTIME_BACKEND — always "native"
	Workspace      string // INTELLIRECON_WORKSPACE — workspace root dir
	DataDir        string // Active per-installation data root. Defaults to ~/.intellirecon/data/. Override via INTELLIRECON_DATA_DIR.
	...
	DisableBrowser bool   // INTELLIRECON_DISABLE_BROWSER
	MaxIterations  int    // INTELLIRECON_MAX_ITERATIONS — 0 = unlimited
	...
	// INTELLIRECON_TARGET_AUTH. Applied automatically to the http_request tool and
	...
	// created by account A using account B's session. INTELLIRECON_TARGET_AUTH_B.
	...
	// targets can reach (e.g. https://oob.intellirecon.com), OOBPort is the local
	// listener port. OOB is enabled only when OOBPublicURL is set.
	// INTELLIRECON_OOB_PUBLIC_URL / INTELLIRECON_OOB_PORT.
	OOBPublicURL string
	OOBPort      int
	...
	// INTELLIRECON_INTERACTSH_SERVER / INTELLIRECON_INTERACTSH_TOKEN / INTELLIRECON_OOB_DISABLE.
	...
	// turning a blind black-box scan into an informed one. INTELLIRECON_SCAN_CONTEXT.
	ScanContext string
	...
	// INTELLIRECON_MAX_TOOL_CALLS / INTELLIRECON_MAX_DURATION (seconds) / INTELLIRECON_MAX_TOKENS.
	MaxToolCalls   int
	MaxDurationSec int
	MaxTokens      int

	// ScanRetentionDays controls automatic pruning of old scan output
	// directories under DataDir. INTELLIRECON_SCAN_RETENTION_DAYS — when > 0, a
	...
	ScanRetentionDays int // INTELLIRECON_SCAN_RETENTION_DAYS — 0 = disabled (keep forever)

	// Rate limiting & API settings
	RateLimitRequests int // INTELLIRECON_RATE_LIMIT_REQUESTS — requests per window
	RateLimitWindow   int // INTELLIRECON_RATE_LIMIT_WINDOW — window in seconds
	...
	// Telemetry
	Telemetry    bool   // INTELLIRECON_TELEMETRY
	OTelEndpoint string // INTELLIRECON_OTEL_ENDPOINT
	...
	DiscordWebhook     string // INTELLIRECON_DISCORD_WEBHOOK - notification webhook URL
	DiscordMinSeverity string // INTELLIRECON_DISCORD_MIN_SEVERITY - minimum severity to notify

	// Telegram notifications
	TelegramBotToken    string // INTELLIRECON_TELEGRAM_BOT_TOKEN - bot token from @BotFather (secret)
	TelegramChatID      string // INTELLIRECON_TELEGRAM_CHAT_ID - target chat/channel ID (numeric or @username)
	TelegramMinSeverity string // INTELLIRECON_TELEGRAM_MIN_SEVERITY - minimum severity to notify

	// Dashboard auth
	Username     string // INTELLIRECON_USERNAME - dashboard login username
	Password     string // INTELLIRECON_PASSWORD - dashboard login password (DEPRECATED: prefer PasswordHash)
	PasswordHash string // INTELLIRECON_PASSWORD_HASH - bcrypt hash of the dashboard password (preferred)
	...
	// INTELLIRECON_BIND=0.0.0.0 (or a specific interface IP) to expose externally —
	// but in that case Username + (Password|PasswordHash) MUST be configured or
	// the server will refuse to start.
	BindAddr string // INTELLIRECON_BIND - listen address (default 127.0.0.1)
	...
	AllowAutoInstall     bool // INTELLIRECON_ALLOW_AUTO_INSTALL - permit package auto-install (default false unless root)
	AllowAutoInstallSudo bool // INTELLIRECON_AUTO_INSTALL_SUDO  - permit sudo-prefixed installs (default false)

	// Proxy settings
	UseProxy      bool   // INTELLIRECON_USE_PROXY — enable proxy support
	ProxyFile     string // INTELLIRECON_PROXY_FILE — path to proxies.txt
	ProxyRotation string // INTELLIRECON_PROXY_ROTATION — "roundrobin" (default) or "random"
	ProxyURL      string // INTELLIRECON_PROXY_URL — single proxy URL (overrides file)

	// Paths
	HomeDir     string // ~/.intellirecon
	SkillsDir   string // embedded or local skills directory
	BrowserPath string // INTELLIRECON_BROWSER_PATH — override auto-download with custom Chrome path
	...
	// INTELLIRECON_READ_DENY_LIST to a colon-separated list of additional
	// roots; defaults are merged in. See sandbox.Policy.CheckRead.
	ReadDenyList []string // INTELLIRECON_READ_DENY_LIST
}
...
func load() *Config {
	...
	loadEnvFile("/etc/intellirecon.env")
	// Try the actual user's home (works even under sudo)
	if sudoUser := os.Getenv("SUDO_USER"); sudoUser != "" {
		loadEnvFile(filepath.Join("/home", sudoUser, ".intellirecon.env"))
	}
	...
	loadEnvFile(filepath.Join(home, ".intellirecon.env"))

	intellireconHome := filepath.Join(home, ".intellirecon")
	...
	// pinned a workspace path; if we sampled INTELLIRECON_DATA_DIR after the
	// shim ran, the shim's synthetic value would always look like an
	// override and we'd never warn legitimate cases.
	originalDataDirEnv := os.Getenv("INTELLIRECON_DATA_DIR")
	originalWorkspaceEnv := os.Getenv("INTELLIRECON_WORKSPACE")
	// Backwards-compat shim for legacy INTELLIRECON_WORKSPACE: if it's set and
	// INTELLIRECON_DATA_DIR is not, treat INTELLIRECON_WORKSPACE as if it were
	// INTELLIRECON_DATA_DIR and emit a deprecation notice. External scripts that
	// pin a workspace path therefore keep working without change.
	if originalWorkspaceEnv != "" && originalDataDirEnv == "" {
		log.Printf("[config] WARN INTELLIRECON_WORKSPACE is deprecated; please use INTELLIRECON_DATA_DIR=%s", originalWorkspaceEnv)
		_ = os.Setenv("INTELLIRECON_DATA_DIR", originalWorkspaceEnv)
	}
	...
	// INTELLIRECON_WORKSPACE env var is honored above as a backwards-compat shim.
	workspace := dataDir

	cfg := &Config{
		// LLM
		LLM:             envOr("INTELLIRECON_LLM", ""),
		APIBase:         envOr("INTELLIRECON_API_BASE", ""),
		APIKey:          envOr("INTELLIRECON_API_KEY", ""),
		LLMProfile:      envOr("INTELLIRECON_LLM_PROFILE", ""),
		ReasoningEffort: envOr("INTELLIRECON_REASONING_EFFORT", "high"),
		Temperature:     envOrFloatPtr("INTELLIRECON_TEMPERATURE", 0.2),
		LLMMaxRetries:   envOrInt("INTELLIRECON_LLM_MAX_RETRIES", 5),
		MemCompTimeout:  envOrInt("INTELLIRECON_MEMORY_COMPRESSOR_TIMEOUT", 30),
		...
		legacyCWD:           cwd,
		DisableBrowser:      envOrBool("INTELLIRECON_DISABLE_BROWSER", false),
		MaxIterations:       envOrInt("INTELLIRECON_MAX_ITERATIONS", 0),
		TargetAuth:          envOr("INTELLIRECON_TARGET_AUTH", ""),
		TargetAuthSecondary: envOr("INTELLIRECON_TARGET_AUTH_B", ""),
		OOBPublicURL:        envOr("INTELLIRECON_OOB_PUBLIC_URL", ""),
		OOBPort:             envOrInt("INTELLIRECON_OOB_PORT", 0),
		InteractshServer:    envOr("INTELLIRECON_INTERACTSH_SERVER", ""),
		InteractshToken:     envOr("INTELLIRECON_INTERACTSH_TOKEN", ""),
		OOBDisable:          envOrBool("INTELLIRECON_OOB_DISABLE", false),
		SourceRepo:          envOr("INTELLIRECON_SOURCE_REPO", ""),
		ScanContext:         envOr("INTELLIRECON_SCAN_CONTEXT", ""),
		MaxToolCalls:        envOrInt("INTELLIRECON_MAX_TOOL_CALLS", 0),
		MaxDurationSec:      envOrInt("INTELLIRECON_MAX_DURATION", 0),
		MaxTokens:           envOrInt("INTELLIRECON_MAX_TOKENS", 0),

		// Scan retention: 0 disables automatic pruning (keep forever).
		ScanRetentionDays: envOrInt("INTELLIRECON_SCAN_RETENTION_DAYS", 0),

		// Rate limiting (defaults: 60 requests per 60 seconds)
		RateLimitRequests: envOrInt("INTELLIRECON_RATE_LIMIT_REQUESTS", 60),
		RateLimitWindow:   envOrInt("INTELLIRECON_RATE_LIMIT_WINDOW", 60),
		RateLimitRPS:      envOrFloat("INTELLIRECON_RATE_RPS", 10),
		RateLimitBurst:    envOrInt("INTELLIRECON_RATE_BURST", 20),
		TLSSkipVerify:     envOrBool("INTELLIRECON_TLS_SKIP_VERIFY", envOrBool("INTELLIRECON_TLS_INSECURE_SKIP_VERIFY", false)),
		...
		// Telemetry
		Telemetry:    envOrBool("INTELLIRECON_TELEMETRY", true),
		OTelEndpoint: envOr("INTELLIRECON_OTEL_ENDPOINT", ""),
		...
		// Discord notifications
		DiscordWebhook:     envOr("INTELLIRECON_DISCORD_WEBHOOK", ""),
		DiscordMinSeverity: envOr("INTELLIRECON_DISCORD_MIN_SEVERITY", ""),

		// Telegram notifications
		TelegramBotToken:    envOr("INTELLIRECON_TELEGRAM_BOT_TOKEN", ""),
		TelegramChatID:      envOr("INTELLIRECON_TELEGRAM_CHAT_ID", ""),
		TelegramMinSeverity: envOr("INTELLIRECON_TELEGRAM_MIN_SEVERITY", ""),

		// Dashboard auth
		Username:     envOr("INTELLIRECON_USERNAME", ""),
		Password:     envOr("INTELLIRECON_PASSWORD", ""),
		PasswordHash: envOr("INTELLIRECON_PASSWORD_HASH", ""),

		// Network binding — loopback-only by default.
		BindAddr: envOr("INTELLIRECON_BIND", "127.0.0.1"),
		...
		AllowAutoInstall:     envOrBool("INTELLIRECON_ALLOW_AUTO_INSTALL", os.Getuid() == 0),
		AllowAutoInstallSudo: envOrBool("INTELLIRECON_AUTO_INSTALL_SUDO", false),

		// Proxy
		UseProxy:      envOrBool("INTELLIRECON_USE_PROXY", false),
		ProxyFile:     envOr("INTELLIRECON_PROXY_FILE", ""),
		ProxyRotation: envOr("INTELLIRECON_PROXY_ROTATION", "roundrobin"),
		ProxyURL:      envOr("INTELLIRECON_PROXY_URL", ""),

		// Paths
		HomeDir:     intellireconHome,
		SkillsDir:   filepath.Join(intellireconHome, "skills"),
		BrowserPath: envOr("INTELLIRECON_BROWSER_PATH", ""),

		// Filesystem read deny-list. Defaults applied in resolveReadDenyList
		// (sensitive home and system paths); user list extends them.
		ReadDenyList: resolveReadDenyList(home, os.Getenv("INTELLIRECON_READ_DENY_LIST")),
	}

	// Debug: show loaded config so users can verify correct env was picked up.
	// Gated behind INTELLIRECON_DEBUG_CONFIG so it doesn't pollute every CLI
	...
	if envOrBool("INTELLIRECON_DEBUG_CONFIG", false) {
		maskedKey := ""
		if len(cfg.APIKey) > 8 {
	...
	// Suppressed by an explicit INTELLIRECON_DATA_DIR or the legacy
	// INTELLIRECON_WORKSPACE override (R7.4); idempotence (R7.3) is provided by
...
func (c *Config) Validate() error {
	...
	if c.DataDir == "" {
		return fmt.Errorf("DataDir is empty — Data_Dir resolution failed; check INTELLIRECON_DATA_DIR or HOME and verify the binary can create ~/.intellirecon/data with mode 0o700")
	}
	if c.LLM == "" {
		return fmt.Errorf("INTELLIRECON_LLM is required. Set it to a model like 'openai/gpt-5.4' or 'anthropic/claude-sonnet-4-20250514'")
	}
	if c.APIKey == "" {
		return fmt.Errorf("INTELLIRECON_API_KEY is required. Set it in ~/.intellirecon.env")
	}
	return nil
}

// CheckEnvFile checks if .intellirecon.env exists and has valid content.
...
// var INTELLIRECON_DATA_DIR is unset it falls back to ~/.intellirecon/data/. Existing
// directories are tightened to 0o700 so we never trust looser ambient perms.
func resolveDataDir(home string) (string, error) {
	raw := os.Getenv("INTELLIRECON_DATA_DIR")
	if raw == "" {
		raw = filepath.Join(home, ".intellirecon", "data")
	}
...
// $CWD-rooted IntelliRecon workspace layout (notes.json, _schedules/,
// vulnerabilities.json, or YYYY-MM-DD/scan-* output dirs) and the user has
// not pinned the workspace via INTELLIRECON_DATA_DIR or INTELLIRECON_WORKSPACE.
//
...
func maybeEmitMigrationWarning(cwd, dataDir string, suppressed bool) {
	...
	log.Printf("[MIGRATION] Detected legacy workspace layout under %s "+
		...
		"INTELLIRECON_DATA_DIR=%s", cwd, found, dataDir, cwd)
}
...
// setting INTELLIRECON_TEMPERATURE=0 sends temperature=0.0 to the API
...
// entries supplied via INTELLIRECON_READ_DENY_LIST. Entries in the env var
