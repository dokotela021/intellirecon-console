...
func TestEnvOr(t *testing.T) {
	...
	os.Unsetenv("TEST_INTELLIRECON_ENVVAR")
	if got := envOr("TEST_INTELLIRECON_ENVVAR", "default_val"); got != "default_val" {
		t.Errorf("expected 'default_val', got '%s'", got)
	}

	// Set env var should return it
	os.Setenv("TEST_INTELLIRECON_ENVVAR", "custom_val")
	defer os.Unsetenv("TEST_INTELLIRECON_ENVVAR")
	if got := envOr("TEST_INTELLIRECON_ENVVAR", "default_val"); got != "custom_val" {
		t.Errorf("expected 'custom_val', got '%s'", got)
	}
}

func TestEnvOrInt(t *testing.T) {
	os.Unsetenv("TEST_INTELLIRECON_INT")
	if got := envOrInt("TEST_INTELLIRECON_INT", 42); got != 42 {
		t.Errorf("expected 42, got %d", got)
	}

	os.Setenv("TEST_INTELLIRECON_INT", "99")
	defer os.Unsetenv("TEST_INTELLIRECON_INT")
	if got := envOrInt("TEST_INTELLIRECON_INT", 42); got != 99 {
		t.Errorf("expected 99, got %d", got)
	}

	// Invalid int should return fallback
	os.Setenv("TEST_INTELLIRECON_INT", "not_a_number")
	if got := envOrInt("TEST_INTELLIRECON_INT", 42); got != 42 {
...
func TestEnvOrBool(t *testing.T) {
	os.Unsetenv("TEST_INTELLIRECON_BOOL")
	if got := envOrBool("TEST_INTELLIRECON_BOOL", false); got != false {
		t.Error("expected false for unset env")
	}

	trueValues := []string{"1", "true", "TRUE", "True", "yes", "YES", "Yes"}
	for _, v := range trueValues {
		os.Setenv("TEST_INTELLIRECON_BOOL", v)
		if got := envOrBool("TEST_INTELLIRECON_BOOL", false); got != true {
...
INTELLIRECON_TEST_KEY1=value1
export INTELLIRECON_TEST_KEY2=value2
INTELLIRECON_TEST_KEY3="quoted_value"
INTELLIRECON_TEST_KEY4='single_quoted'

# Another comment
INTELLIRECON_TEST_KEY5=with=equals=signs
`
	...
	defer func() {
		for i := 1; i <= 5; i++ {
			os.Unsetenv("INTELLIRECON_TEST_KEY" + string(rune('0'+i)))
		}
	}()
...
func TestLoad_ReadsDashboardProviderProxyAndAgentMailSettings(t *testing.T) {
	...
	t.Setenv("HOME", home)
	t.Setenv("SUDO_USER", "")
	t.Setenv("INTELLIRECON_DEBUG_CONFIG", "")

	envFile := filepath.Join(home, ".intellirecon.env")
	content := strings.Join([]string{
		"INTELLIRECON_LLM=google/gemini-3.1-pro-preview",
		"INTELLIRECON_API_BASE=https://generativelanguage.googleapis.com/v1",
		"INTELLIRECON_API_KEY=gemini-key",
		"INTELLIRECON_REASONING_EFFORT=medium",
		"INTELLIRECON_LLM_MAX_RETRIES=2",
		"INTELLIRECON_MEMORY_COMPRESSOR_TIMEOUT=45",
		"INTELLIRECON_WORKSPACE=/tmp/intellirecon-workspace",
		"INTELLIRECON_DISABLE_BROWSER=true",
		"INTELLIRECON_MAX_ITERATIONS=12",
		"INTELLIRECON_RATE_LIMIT_REQUESTS=7",
		"INTELLIRECON_RATE_LIMIT_WINDOW=11",
		"INTELLIRECON_RATE_RPS=2.5",
		"INTELLIRECON_RATE_BURST=9",
		"INTELLIRECON_TLS_SKIP_VERIFY=true",
		"CAIDO_PORT=9090",
		"CAIDO_API_TOKEN=caido-token",
		"INTELLIRECON_TELEMETRY=false",
		"INTELLIRECON_OTEL_ENDPOINT=http://otel.test",
		"GEMINI_API_KEY=search-key",
		"AGENTMAIL_API_KEY=agentmail-key",
		"AGENTMAIL_POD=am_test_pod",
		"INTELLIRECON_DISCORD_WEBHOOK=https://discord.example/webhook",
		"INTELLIRECON_DISCORD_MIN_SEVERITY=high",
		"INTELLIRECON_TELEGRAM_BOT_TOKEN=123456:ABC-DEF",
		"INTELLIRECON_TELEGRAM_CHAT_ID=-1001234567890",
		"INTELLIRECON_TELEGRAM_MIN_SEVERITY=critical",
		"INTELLIRECON_USERNAME=admin",
		"INTELLIRECON_PASSWORD=password",
		"INTELLIRECON_BROWSER_PATH=/opt/chrome",
	}, "\n")
	...
	if cfg.Workspace != "/tmp/intellirecon-workspace" || !cfg.DisableBrowser || cfg.MaxIterations != 12 {
	...
	if cfg.HomeDir != filepath.Join(home, ".intellirecon") || cfg.SkillsDir != filepath.Join(home, ".intellirecon", "skills") {
...
func TestConfig_Validate(t *testing.T) {
	...
	cfg.DataDir = "/tmp/intellirecon-validate-test"
	if err := cfg.Validate(); err == nil {
...
func TestConfig_WorkspacePath(t *testing.T) {
	...
	drift := &Config{
		...
		DataDir:       "/home/user/.intellirecon/data",
		WorkspaceRoot: "/home/user/.intellirecon/data",
	}
	if got := drift.WorkspacePath("subdir/file.txt"); got != "/home/user/.intellirecon/data/subdir/file.txt" {
