...
// for outbound HTTP requests made by IntelliRecon tools.
//
// Configuration (via env or ~/.intellirecon.env):
//
//	INTELLIRECON_RATE_RPS   – sustained requests per second per domain (default 10)
//	INTELLIRECON_RATE_BURST – burst size per domain                    (default 20)
//
// Set INTELLIRECON_RATE_RPS=0 to disable rate limiting entirely.
package ratelimit
