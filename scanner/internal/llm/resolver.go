...
type catalogResolver struct {
	...
	// (cfg.APIBase / INTELLIRECON_API_BASE) used when neither the
...
func defaultCatalogPick(c *compositeResolver) func(ctx context.Context) (catalogPick, error) {
	return func(ctx context.Context) (catalogPick, error) {
		...
		if c.cfg == nil || strings.TrimSpace(c.cfg.LLMProfile) == "" {
			return catalogPick{}, &ConfigError{Msg: "catalog resolver: no active LLM profile (INTELLIRECON_LLM_PROFILE unset)"}
		}
...
func (c *compositeResolver) Resolve(ctx context.Context) (Endpoint, error) {
	...
	if c.cfg != nil && strings.TrimSpace(c.cfg.LLMProfile) != "" && c.cat != nil && c.prof != nil {
		...
		if err == nil {
			...
			// operator's configured model (INTELLIRECON_LLM) stripped
	...
	return Endpoint{}, &ConfigError{
		Msg: "no provider configured: set INTELLIRECON_LLM_PROFILE to a saved credential or set INTELLIRECON_LLM + INTELLIRECON_API_KEY",
	}
}
...
// INTELLIRECON_API_BASE) consulted only when neither the catalog Entry
...
func BuildCatalogEndpoint(entry providers.Entry, prof auth.Profile, preferModel, fallbackBase string) (Endpoint, error) {
	...
	if parsed, perr := url2.Parse(url); perr != nil || parsed.Scheme == "" || parsed.Host == "" {
		return Endpoint{}, &ConfigError{
			Msg: "catalog resolver: provider " + entry.ID + " has no base URL — set the Base URL for the Custom Provider (or INTELLIRECON_API_BASE); built non-absolute request URL " + strconv.Quote(url),
		}
	}
