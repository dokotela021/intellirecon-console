...
func TestContextFileEditorStaysInsideScanWorkspace(t *testing.T) {
	...
	// A write target outside every Allow_List root (Data_Dir, ~/.intellirecon, /tmp)
	...
	res, err := strReplaceEditorForContext(sc.ID, map[string]string{
		"command":   "create",
		"path":      "/etc/intellirecon-fileedit-test-outside.txt",
		"file_text": "outside",
	})
	if err != nil {
	...
	if _, statErr := os.Stat("/etc/intellirecon-fileedit-test-outside.txt"); statErr == nil {
