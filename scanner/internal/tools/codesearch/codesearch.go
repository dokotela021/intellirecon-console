...
func executeWithContext(contextID string, args map[string]string) (tools.Result, error) {
	...
	if root == "" {
		return tools.Result{Output: "❌ Whitebox source not available for this scan (set INTELLIRECON_SOURCE_REPO to a git URL or local path). Fall back to black-box testing (fetch and read client-side JS bundles with http_request/browser to discover endpoints)."}, nil
	}
