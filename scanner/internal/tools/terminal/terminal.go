...
import (
	...
	"intellirecon-scanner/internal/tools/iolimit"
)
...
func requestRatePolicyForContext(contextID string) scanctx.RequestRatePolicy {
	...
	if cfg := config.Get(); cfg != nil && cfg.RateLimitRPS > 0 {
		return scanctx.NormalizeRequestRatePolicy(scanctx.RequestRatePolicy{MaxRPS: cfg.RateLimitRPS, Source: "INTELLIRECON_RATE_RPS"})
	}
...
func prepareRequestRateRuntime(workDir string, policy scanctx.RequestRatePolicy) (requestRateRuntime, error) {
	...
	root := filepath.Join(workDir, ".intellirecon-rate")
...
set -euo pipefail
lock_dir="${INTELLIRECON_RATE_LOCK_DIR:-/tmp/intellirecon-rate}"
delay_ms="${INTELLIRECON_RATE_DELAY_MS:-0}"
mkdir -p "$lock_dir"
...
_delay_ms = int(os.environ.get("INTELLIRECON_RATE_DELAY_MS", "0") or "0")
_lock_dir = os.environ.get("INTELLIRECON_RATE_LOCK_DIR", "/tmp/intellirecon-rate")
_patched = {}
...
// - Adjusts OOM score so the kernel kills scan tools before intellirecon
...
func setProcessLimitsForPID(pid int, memoryLimited bool, memLimitBytes int64) {
	...
	// intellirecon protects itself with a negative score, so the kernel prefers
	...
	// If the tool exceeds this, it gets ENOMEM / SIGSEGV — intellirecon survives.
...
func killTrackedProcess(cmd *exec.Cmd) {
	...
	if pgid, err := syscall.Getpgid(pid); err == nil && pgid > 0 {
		...
		if selfErr != nil || pgid != selfPGID {
			...
			log.Printf("[terminal] Refusing to kill process group %d for pid %d because it matches intellirecon", pgid, pid)
		}
	}
...
func executeCommandWithContextID(contextID string, args map[string]string) (tools.Result, error) {
	...
	if reason := isBlockedCommand(command); reason != "" {
		return tools.Result{Output: fmt.Sprintf("[BLOCKED] Destructive command rejected: %s. IntelliRecon is read-only — it tests for vulnerabilities without causing damage.", reason)}, nil
	}
...
//  4. /tmp/intellirecon-workspace as a last-resort sentinel.
//
// $CWD is intentionally NOT consulted: the previous os.Getwd() fallback was
// the source of the workspace-leak bug where running intellirecon from a source
...
func effectiveWorkDirForContext(contextID string, cfg *config.Config) string {
	...
	if workDir == "" {
		workDir = "/tmp/intellirecon-workspace"
	}
...
// XDG_DATA_HOME, INTELLIRECON_WORKSPACE) is rooted at workDir, which the caller
...
func commandEnv(homeDir, goPath, workDir string, rateRuntime requestRateRuntime) []string {
	...
	replace := map[string]bool{
		...
		"XDG_DATA_HOME":          true,
		"INTELLIRECON_WORKSPACE":     true,
		"PYTHONPATH":             true,
		"INTELLIRECON_RATE_LOCK_DIR": true,
		"INTELLIRECON_RATE_DELAY_MS": true,
	}
	...
	env = append(env,
		...
		"INTELLIRECON_WORKSPACE="+workDir,
	)
	if rateRuntime.PythonPath != "" {
		...
		env = append(env,
			...
			"INTELLIRECON_RATE_LOCK_DIR="+rateRuntime.LockDir,
			"INTELLIRECON_RATE_DELAY_MS="+strconv.Itoa(rateRuntime.DelayMS),
		)
...
func shellPrelude(homeDir, workDir string, memLimitBytes int64) string {
	...
	fmt.Fprintf(&b, "export INTELLIRECON_WORKSPACE=%s\n", shellQuote(workDir))
	b.WriteString(`__intellirecon_workspace="$(pwd -P)"
__intellirecon_resolve_path() {
  realpath -m "$1" 2>/dev/null || readlink -m "$1" 2>/dev/null || printf '%s\n' "$1"
}
cd() {
  local dest target resolved before after
  if [ "$#" -eq 0 ]; then
    dest="$__intellirecon_workspace"
  else
    dest="$1"
  fi
  if [ "$dest" = "-" ]; then
    ...
    after="$(pwd -P)"
    case "$after" in
      "$__intellirecon_workspace"|"$__intellirecon_workspace"/*) pwd; return ;;
      *) builtin cd "$before"; printf '[WORKSPACE GUARD] cd outside scan workspace blocked: %s\n' "$dest" >&2; return 1 ;;
    esac
  fi
  case "$dest" in
    "~") target="$__intellirecon_workspace" ;;
    "~/"*) target="$__intellirecon_workspace/${dest#~/}" ;;
    /*) target="$dest" ;;
    *) target="$PWD/$dest" ;;
  esac
  resolved="$(__intellirecon_resolve_path "$target")"
  case "$resolved" in
    "$__intellirecon_workspace"|"$__intellirecon_workspace"/*) builtin cd "$resolved" ;;
...
// has explicitly opted in via INTELLIRECON_AUTO_INSTALL_SUDO=1. The previous
// behavior was to silently sudo any install attempt, which is a privilege
// escalation surface when intellirecon is launched by a user with passwordless
...
// INTELLIRECON_ALLOW_AUTO_INSTALL — defaults to true for root and false for
// non-root, so a stock unprivileged intellirecon invocation can never call
...
func installPackage(pkg string) string {
	...
	if !cfg.AllowAutoInstall {
		return fmt.Sprintf(
			"[install %s blocked: auto-install is disabled. Set INTELLIRECON_ALLOW_AUTO_INSTALL=1 in ~/.intellirecon.env to enable, or install manually.]",
			pkg,
		)
	}
	...
	// INTELLIRECON_AUTO_INSTALL_SUDO=1. Without that, this install will fail
...
//
// The real isolation must be operational: run intellirecon as an unprivileged
