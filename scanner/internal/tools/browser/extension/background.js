// background.js — MV3 Service Worker for IntelliRecon extension.
...
function connect() {
  ...
  try {
    ...
    console.warn("[intellirecon-bg] WebSocket constructor failed:", e.message);
    scheduleReconnect();
    return;
  }

  ws.onopen = () => {
    console.log("[intellirecon-bg] Connected to agent");
    reconnectAttempts = 0;
  ...
  ws.onmessage = (event) => {
    ...
    try {
      ...
      console.error("[intellirecon-bg] Invalid JSON from agent:", event.data);
      return;
    }
  ...
  ws.onerror = (event) => {
    console.warn("[intellirecon-bg] WebSocket error");
  };

  ws.onclose = (event) => {
    console.log(`[intellirecon-bg] Disconnected (code=${event.code})`);
    ws = null;
    scheduleReconnect();
  };
}
...
function scheduleReconnect() {
  ...
  reconnectAttempts++;
  console.log(`[intellirecon-bg] Reconnecting in ${delay}ms (attempt ${reconnectAttempts})`);
...
async function handleAgentCommand(msg) {
  ...
  if (!requestId || !command) {
    console.warn("[intellirecon-bg] Invalid command (missing requestId/command):", msg);
    return;
  }
...
chrome.alarms.create("intellirecon-keepalive", { periodInMinutes: 0.3 }); // ~18s

chrome.alarms.onAlarm.addListener((alarm) => {
  if (alarm.name === "intellirecon-keepalive") {
...
chrome.runtime.onInstalled.addListener(() => {
  console.log("[intellirecon-bg] Extension installed/updated");
  connect();
});
