...
// ~/.intellirecon/browser/ so the default policy roots admit it without any
...
// Priority: 1) INTELLIRECON_BROWSER_PATH env  2) System-installed browser  3) Rod auto-download (~170MB first run)
func getChromiumPath(ctxID string) (string, error) {
	...
	if p := config.Get().BrowserPath; p != "" {
		...
		return "", fmt.Errorf("INTELLIRECON_BROWSER_PATH set to %q but file not found", p)
	}
...
// (~/.intellirecon/browser/extension) and every write is canonicalized through
...
func ensureBrowser(ctxID, proxy string) error {
	...
	// should be reaped before intellirecon under pressure.
	pid := ln.PID()
	if pid > 0 {
		activeLease := lease
