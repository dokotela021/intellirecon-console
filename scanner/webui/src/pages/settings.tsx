...
export default function SettingsPage() {
  ...
  useEffect(() => {
    const webhook = envValue(environment.data, "INTELLIRECON_DISCORD_WEBHOOK");
    const minSeverity = envValue(environment.data, "INTELLIRECON_DISCORD_MIN_SEVERITY");
    const telegramBotToken = envValue(environment.data, "INTELLIRECON_TELEGRAM_BOT_TOKEN");
    const telegramChatId = envValue(environment.data, "INTELLIRECON_TELEGRAM_CHAT_ID");
    const telegramMinSeverity = envValue(environment.data, "INTELLIRECON_TELEGRAM_MIN_SEVERITY");
  ...
  async function saveLLMSettings() {
    ...
    if (llmForm.authMethod === "oauth" && llmForm.provider) {
      ...
      // credential pointer (INTELLIRECON_LLM_PROFILE) at that profile —
          ...
          {llm.isLoading || providers.isLoading ? (
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Saved to {llmForm.envFile || "~/.intellirecon.env"} and used by new scans.
                </CardDescription>
              </CardHeader>
                  ...
                  <Button
                    onClick={async () => {
                      ...
                      await updateEnvironment.mutateAsync({
                        INTELLIRECON_DISCORD_WEBHOOK: notificationForm.webhook,
                        INTELLIRECON_DISCORD_MIN_SEVERITY: notificationForm.minSeverity,
                        INTELLIRECON_TELEGRAM_BOT_TOKEN: notificationForm.telegramBotToken,
                        INTELLIRECON_TELEGRAM_CHAT_ID: notificationForm.telegramChatId,
                        INTELLIRECON_TELEGRAM_MIN_SEVERITY: notificationForm.telegramMinSeverity,
                      });
                  ...
                  <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                    <div>
                      ...
                      <p className="mt-1 text-xs text-muted-foreground">
                        Editing {environment.data?.envFile || "~/.intellirecon.env"}. Masked secrets are preserved unless you replace or clear them.
                      </p>
                    </div>
