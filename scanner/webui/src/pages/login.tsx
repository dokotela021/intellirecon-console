...
function describeLoginError(err: unknown): string {
  if (err instanceof HttpError) {
    ...
    switch (err.status) {
      case 0:
        return "Cannot reach the server. Check your network or that the IntelliRecon service is running."
      case 400:
      ...
      case 404:
        ...
        return "Login endpoint not found on this server. The backend appears to be out of date — rebuild and redeploy the IntelliRecon server."
      case 405:
    ...
    <div className="grid min-h-screen w-full bg-background lg:grid-cols-2">
      <div className="hidden flex-col justify-between border-r border-border bg-muted/30 p-12 lg:flex">
        <div className="flex items-center gap-3">
          <img src="/logo.png" alt="IntelliRecon" className="h-8 w-8 rounded-md" />
          <span className="font-mono text-sm font-semibold tracking-tight">INTELLIRECON</span>
        </div>
        ...
        <p className="text-xs text-muted-foreground">© IntelliRecon · Internal use only</p>
      </div>
