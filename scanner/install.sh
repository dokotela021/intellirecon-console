...
#
#   curl -sSL https://raw.githubusercontent.com/dokotela021/intellirecon-console/master/scanner/install.sh | bash
#
# What it does:
...
#
# Env overrides:
#   INTELLIRECON_INSTALL_DIR   target directory (default: /usr/local/bin)
#   INTELLIRECON_VERSION       specific version tag, e.g. v4.5.48 (default: latest)
#
set -euo pipefail
...
case "$OS" in
  Linux) OS="linux" ;;
  *) die "Unsupported OS '$OS'. IntelliRecon ships Linux binaries; build from source for other platforms (see README)." ;;
esac
...
# ── Install ─────────────────────────────────────────────────────────────────
DEST="${INTELLIRECON_INSTALL_DIR:-/usr/local/bin}"
...
echo ""
echo "  Next steps:"
echo "    1. Set your LLM provider in ~/.intellirecon.env:"
echo "         INTELLIRECON_LLM=anthropic/claude-opus-4-8"
echo "         INTELLIRECON_API_KEY=your_provider_api_key"
