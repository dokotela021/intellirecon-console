...
| `INTELLIRECON_DATA_DIR` | — | Scan output / reports location. Defaults to `/data` (mount a volume). |
...
- **Never expose the dashboard publicly without `INTELLIRECON_USERNAME`/`INTELLIRECON_PASSWORD`.** The server refuses external binding without auth.
