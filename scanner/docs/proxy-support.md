...
IntelliRecon supports routing all outbound network requests through **HTTP**, **HTTPS**, and **SOCKS5** proxies, with automatic rotation from a proxy list.
...
export INTELLIRECON_USE_PROXY=true
export INTELLIRECON_PROXY_URL="1.2.3.4:8080"

# HTTP / HTTPS proxy — with auth
export INTELLIRECON_USE_PROXY=true
export INTELLIRECON_PROXY_URL="1.2.3.4:8080:username:password"

# SOCKS5 proxy
export INTELLIRECON_USE_PROXY=true
export INTELLIRECON_PROXY_URL="socks5://10.0.0.1:1080"

# SOCKS5 with auth
export INTELLIRECON_USE_PROXY=true
export INTELLIRECON_PROXY_URL="socks5://user:pass@10.0.0.1:1080"
```
...
```bash
export INTELLIRECON_USE_PROXY=true
export INTELLIRECON_PROXY_FILE="/path/to/proxies.txt"
export INTELLIRECON_PROXY_ROTATION="roundrobin"   # or "random"
```
...
|---------------------------|----------------|--------------------------------------------------------------|
| `INTELLIRECON_USE_PROXY`      | `false`        | Set to `true` to enable proxy routing                        |
| `INTELLIRECON_PROXY_URL`      | _(empty)_      | Single proxy string; takes precedence over `PROXY_FILE`      |
| `INTELLIRECON_PROXY_FILE`     | _(empty)_      | Path to a file with one proxy per line                       |
| `INTELLIRECON_PROXY_ROTATION` | `roundrobin`   | Rotation strategy: `roundrobin` or `random`                  |

All variables can also be placed in `~/.intellirecon.env`.
