...
func TestServiceUnitUsesDedicatedWorkspaceAndContinuesAfterChildOOM(t *testing.T) {
	unit := serviceUnitContent("/root", "/usr/local/bin/intellirecon")
	...
	// Task 3.6 / R6.4: the unit must NOT pin the legacy INTELLIRECON_WORKSPACE
	// env var — the active workspace root is owned by
	// config.resolveDataDir (INTELLIRECON_DATA_DIR / ~/.intellirecon/data).
	if strings.Contains(unit, "INTELLIRECON_WORKSPACE") {
		t.Fatalf("service unit must not set INTELLIRECON_WORKSPACE (deprecated by Task 3.6):\n%s", unit)
	}
}
...
func TestServiceAccessLinesExternalWithoutAuthWarns(t *testing.T) {
	...
	if !strings.Contains(lines, "external bind requires INTELLIRECON_USERNAME") ||
