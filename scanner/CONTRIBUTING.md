# Contributing to IntelliRecon

Thanks for your interest in contributing. IntelliRecon is a self-hosted,
autonomous AI pentesting engine: a single Go binary (`intellirecon`) that serves
...
- **Go 1.25+** — building and testing the `intellirecon` binary.
...
```bash
make build          # builds webui, then the binary into ./build/intellirecon
...
| Path                 | Purpose                                                        |
| -------------------- | -------------------------------------------------------------- |
| `cmd/intellirecon/`      | CLI entry point and service lifecycle (`--web`, `--start`, …). |
