...
- **Model fallback (OpenRouter only)** — `ANTHROPIC_FALLBACK_MODELS`, a comma-separated list of up to 3 model slugs. When the primary model errors (throttled, down, moderation-flagged), OpenRouter transparently retries the same request on the next model in the list before the error ever reaches chat. Free OpenRouter models share one account-wide 16-req/min quota, so chaining free → free doesn't help a rate limit — put a paid model here as the real safety net (e.g. `ANTHROPIC_FALLBACK_MODELS=deepseek/deepseek-chat-v3.1`). When a fallback actually serves a turn, the backend logs `fallback served this turn: <primary> -> <served>`.
