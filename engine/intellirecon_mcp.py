#!/usr/bin/env python3
"""
IntelliRecon AI MCP Client - Enhanced AI Agent Communication Interface
...
Architecture: MCP Client for AI agent communication with IntelliRecon server
...
Colors = IntelliReconColors

class ColoredFormatter(logging.Formatter):
    """Enhanced formatter with colors and emojis for MCP client - matches server styling"""

    COLORS = {
        'DEBUG': IntelliReconColors.DEBUG,
        'INFO': IntelliReconColors.SUCCESS,
        'WARNING': IntelliReconColors.WARNING,
        'ERROR': IntelliReconColors.ERROR,
        'CRITICAL': IntelliReconColors.CRITICAL
    }
    ...
    def format(self, record):
        ...
        color = self.COLORS.get(record.levelname, IntelliReconColors.BRIGHT_WHITE)

        # Add color and emoji to the message
        record.msg = f"{color}{emoji} {record.msg}{IntelliReconColors.RESET}"
        return super().format(record)

# Setup logging
logging.basicConfig(
    level=logging.INFO,
    format="[🔥 IntelliRecon MCP] %(asctime)s [%(levelname)s] %(message)s",
    handlers=[
...
for handler in logging.getLogger().handlers:
    handler.setFormatter(ColoredFormatter(
        "[🔥 IntelliRecon MCP] %(asctime)s [%(levelname)s] %(message)s",
        datefmt="%Y-%m-%d %H:%M:%S"
    ))
...
DEFAULT_INTELLIRECON_SERVER = "http://127.0.0.1:8888"  # Default IntelliRecon server URL
DEFAULT_REQUEST_TIMEOUT = 300  # 5 minutes default timeout for API requests
MAX_RETRIES = 3  # Maximum number of retries for connection attempts

class IntelliReconClient:
    """Enhanced client for communicating with the IntelliRecon AI API Server"""

    def __init__(self, server_url: str, timeout: int = DEFAULT_REQUEST_TIMEOUT):
        """
        Initialize the IntelliRecon AI Client

        Args:
            server_url: URL of the IntelliRecon AI API Server
        ...
        for i in range(MAX_RETRIES):
            try:
                logger.info(f"🔗 Attempting to connect to IntelliRecon AI API at {server_url} (attempt {i+1}/{MAX_RETRIES})")
                ...
                try:
                    ...
                    connected = True
                    logger.info(f"🎯 Successfully connected to IntelliRecon AI API Server at {server_url}")
                    ...
                    logger.warning(f"🔌 Connection refused to {server_url}. Make sure the IntelliRecon AI server is running.")
        ...
        if not connected:
            error_msg = f"Failed to establish connection to IntelliRecon AI API Server at {server_url} after {MAX_RETRIES} attempts"
    ...
    def execute_command(self, command: str, use_cache: bool = True) -> Dict[str, Any]:
        """
        Execute a generic command on the IntelliRecon server
    ...
    def check_health(self) -> Dict[str, Any]:
        """
        Check the health of the IntelliRecon AI API Server
...
def setup_mcp_server(intellirecon_client: IntelliReconClient) -> FastMCP:
    """
    Set up the MCP server with all enhanced tool functions

    Args:
        intellirecon_client: Initialized IntelliReconClient
    ...
    def nmap_scan(target: str, scan_type: str = "-sV", ports: str = "", additional_args: str = "") -> Dict[str, Any]:
        ...
        logger.info(f"{IntelliReconColors.FIRE_RED}🔍 Initiating Nmap scan: {target}{IntelliReconColors.RESET}")
        ...
        if result.get("success"):
            ...
            if result.get("recovery_info", {}).get("recovery_applied"):
                ...
                logger.info(f"{IntelliReconColors.HIGHLIGHT_YELLOW} Recovery applied: {attempts} attempts made {IntelliReconColors.RESET}")
        else:
            logger.error(f"{IntelliReconColors.ERROR}❌ Nmap scan failed for {target}{IntelliReconColors.RESET}")

            # Check for human escalation
            if result.get("human_escalation"):
                logger.error(f"{IntelliReconColors.CRITICAL} HUMAN ESCALATION REQUIRED {IntelliReconColors.RESET}")
    ...
    def gobuster_scan(url: str, mode: str = "dir", wordlist: str = "/usr/share/wordlists/dirb/common.txt", additional_args: str = "") -> Dict[str, Any]:
        ...
        logger.info(f"{IntelliReconColors.CRIMSON}📁 Starting Gobuster {mode} scan: {url}{IntelliReconColors.RESET}")
        ...
        if result.get("success"):
            ...
            logger.error(f"{IntelliReconColors.ERROR}❌ Gobuster scan failed for {url}{IntelliReconColors.RESET}")

            # Check for alternative tool suggestion
            if result.get("alternative_tool_suggested"):
                alt_tool = result["alternative_tool_suggested"]
                logger.info(f"{IntelliReconColors.HIGHLIGHT_BLUE} Alternative tool suggested: {alt_tool} {IntelliReconColors.RESET}")
    ...
    def nuclei_scan(target: str, severity: str = "", tags: str = "", template: str = "", additional_args: str = "") -> Dict[str, Any]:
        ...
        logger.info(f"{IntelliReconColors.BLOOD_RED}🔬 Starting Nuclei vulnerability scan: {target}{IntelliReconColors.RESET}")
        ...
        if result.get("success"):
            ...
            if result.get("stdout") and "CRITICAL" in result["stdout"]:
                logger.warning(f"{IntelliReconColors.CRITICAL} CRITICAL vulnerabilities detected! {IntelliReconColors.RESET}")
            elif result.get("stdout") and "HIGH" in result["stdout"]:
                logger.warning(f"{IntelliReconColors.FIRE_RED} HIGH severity vulnerabilities found! {IntelliReconColors.RESET}")
            ...
            logger.error(f"{IntelliReconColors.ERROR}❌ Nuclei scan failed for {target}{IntelliReconColors.RESET}")
    ...
    def create_file(filename: str, content: str, binary: bool = False) -> Dict[str, Any]:
        """
        Create a file with specified content on the IntelliRecon server.
        ...
        result = intellirecon_client.safe_post("api/files/create", data)
        if result.get("success"):
    ...
    def modify_file(filename: str, content: str, append: bool = False) -> Dict[str, Any]:
        """
        Modify an existing file on the IntelliRecon server.
        ...
        result = intellirecon_client.safe_post("api/files/modify", data)
        if result.get("success"):
    ...
    def delete_file(filename: str) -> Dict[str, Any]:
        """
        Delete a file or directory on the IntelliRecon server.
        ...
        result = intellirecon_client.safe_post("api/files/delete", data)
        if result.get("success"):
    ...
    def list_files(directory: str = ".") -> Dict[str, Any]:
        """
        List files in a directory on the IntelliRecon server.
        ...
        result = intellirecon_client.safe_get("api/files/list", {"directory": directory})
        if result.get("success"):
    ...
    def generate_payload(payload_type: str = "buffer", size: int = 1024, pattern: str = "A", filename: str = "") -> Dict[str, Any]:
        ...
        result = intellirecon_client.safe_post("api/payloads/generate", data)
        if result.get("success"):
    ...
    def install_python_package(package: str, env_name: str = "default") -> Dict[str, Any]:
        """
        Install a Python package in a virtual environment on the IntelliRecon server.
        ...
        result = intellirecon_client.safe_post("api/python/install", data)
        if result.get("success"):
    ...
    def execute_python_script(script: str, env_name: str = "default", filename: str = "") -> Dict[str, Any]:
        """
        Execute a Python script in a virtual environment on the IntelliRecon server.
        ...
        result = intellirecon_client.safe_post("api/python/execute", data)
        if result.get("success"):
    ...
    def ai_generate_payload(attack_type: str, complexity: str = "basic", technology: str = "", url: str = "") -> Dict[str, Any]:
        ...
        result = intellirecon_client.safe_post("api/ai/generate_payload", data)
    ...
    def ai_test_payload(payload: str, target_url: str, method: str = "GET") -> Dict[str, Any]:
        ...
        result = intellirecon_client.safe_post("api/ai/test_payload", data)
    ...
    def server_health() -> Dict[str, Any]:
        """
        Check the health status of the IntelliRecon AI server.

        Returns:
            Server health information with tool availability and telemetry
        """
        logger.info(f"🏥 Checking IntelliRecon AI server health")
        result = intellirecon_client.check_health()
    ...
    def get_cache_stats() -> Dict[str, Any]:
        """
        Get cache statistics from the IntelliRecon AI server.

        Returns:
            Cache performance statistics
        """
        logger.info(f"💾 Getting cache statistics")
        result = intellirecon_client.safe_get("api/cache/stats")
    ...
    def clear_cache() -> Dict[str, Any]:
        """
        Clear the cache on the IntelliRecon AI server.

        Returns:
            Cache clear operation results
        """
        logger.info(f"🧹 Clearing server cache")
        result = intellirecon_client.safe_post("api/cache/clear", {})
        if result.get("success"):
    ...
    def get_telemetry() -> Dict[str, Any]:
        """
        Get system telemetry from the IntelliRecon AI server.

        Returns:
            System performance and usage telemetry
        """
        logger.info(f"📈 Getting system telemetry")
        result = intellirecon_client.safe_get("api/telemetry")
    ...
    def list_active_processes() -> Dict[str, Any]:
        """
        List all active processes on the IntelliRecon AI server.

        Returns:
            List of active processes with their status and progress
        """
        logger.info("📊 Listing active processes")
        result = intellirecon_client.safe_get("api/processes/list")
        if result.get("success"):
    ...
    def get_process_status(pid: int) -> Dict[str, Any]:
        ...
        result = intellirecon_client.safe_get(f"api/processes/status/{pid}")
        if result.get("success"):
    ...
    def terminate_process(pid: int) -> Dict[str, Any]:
        ...
        result = intellirecon_client.safe_post(f"api/processes/terminate/{pid}", {})
        if result.get("success"):
    ...
    def pause_process(pid: int) -> Dict[str, Any]:
        ...
        result = intellirecon_client.safe_post(f"api/processes/pause/{pid}", {})
        if result.get("success"):
    ...
    def resume_process(pid: int) -> Dict[str, Any]:
        ...
        result = intellirecon_client.safe_post(f"api/processes/resume/{pid}", {})
        if result.get("success"):
    ...
    def get_process_dashboard() -> Dict[str, Any]:
        ...
        result = intellirecon_client.safe_get("api/processes/dashboard")
    ...
    def execute_command(command: str, use_cache: bool = True) -> Dict[str, Any]:
        """
        Execute an arbitrary command on the IntelliRecon AI server with enhanced logging.
        ...
        try:
            ...
            result = intellirecon_client.execute_command(command, use_cache)
            if "error" in result:
    ...
    def monitor_cve_feeds(hours: int = 24, severity_filter: str = "HIGH,CRITICAL", keywords: str = "") -> Dict[str, Any]:
        ...
        result = intellirecon_client.safe_post("api/vuln-intel/cve-monitor", data)
    ...
    def generate_exploit_from_cve(cve_id: str, target_os: str = "", target_arch: str = "x64", exploit_type: str = "poc", evasion_level: str = "none") -> Dict[str, Any]:
        ...
        result = intellirecon_client.safe_post("api/vuln-intel/exploit-generate", data)
    ...
    def discover_attack_chains(target_software: str, attack_depth: int = 3, include_zero_days: bool = False) -> Dict[str, Any]:
        ...
        result = intellirecon_client.safe_post("api/vuln-intel/attack-chains", data)
    ...
    def research_zero_day_opportunities(target_software: str, analysis_depth: str = "standard", source_code_url: str = "") -> Dict[str, Any]:
        ...
        result = intellirecon_client.safe_post("api/vuln-intel/zero-day-research", data)
    ...
    def correlate_threat_intelligence(indicators: str, timeframe: str = "30d", sources: str = "all") -> Dict[str, Any]:
        ...
        result = intellirecon_client.safe_post("api/vuln-intel/threat-feeds", data)
    ...
    def advanced_payload_generation(attack_type: str, target_context: str = "", evasion_level: str = "standard", custom_constraints: str = "") -> Dict[str, Any]:
        ...
        result = intellirecon_client.safe_post("api/ai/advanced-payload-generation", data)
    ...
    def vulnerability_intelligence_dashboard() -> Dict[str, Any]:
        ...
        latest_cves = intellirecon_client.safe_post("api/vuln-intel/cve-monitor", {
            "hours": 24,
        ...
        trending_research = intellirecon_client.safe_post("api/vuln-intel/zero-day-research", {
    ...
    def create_vulnerability_report(vulnerabilities: str, target: str = "", scan_type: str = "comprehensive") -> Dict[str, Any]:
        ...
        try:
            ...
            for vuln in vuln_data:
                card_result = intellirecon_client.safe_post("api/visual/vulnerability-card", vuln)
            ...
            summary_result = intellirecon_client.safe_post("api/visual/summary-report", summary_data)
    ...
    def format_tool_output_visual(tool_name: str, output: str, success: bool = True) -> Dict[str, Any]:
        ...
        result = intellirecon_client.safe_post("api/visual/tool-output", data)
        if result.get("success"):
    ...
    def create_scan_summary(target: str, tools_used: str, vulnerabilities_found: int = 0,
        ...
        result = intellirecon_client.safe_post("api/visual/summary-report", summary_data)
        if result.get("success"):
    ...
    def display_system_metrics() -> Dict[str, Any]:
        ...
        # Get telemetry data
        telemetry_result = intellirecon_client.safe_get("api/telemetry")
...
""",
    ...
    def analyze_target_intelligence(target: str) -> Dict[str, Any]:
        ...
        data = {"target": target}
        result = intellirecon_client.safe_post("api/intelligence/analyze-target", data)
    ...
    def select_optimal_tools_ai(target: str, objective: str = "comprehensive") -> Dict[str, Any]:
        ...
        result = intellirecon_client.safe_post("api/intelligence/select-tools", data)
    ...
    def optimize_tool_parameters_ai(target: str, tool: str, context: str = "{}") -> Dict[str, Any]:
        ...
        result = intellirecon_client.safe_post("api/intelligence/optimize-parameters", data)
    ...
    def create_attack_chain_ai(target: str, objective: str = "comprehensive") -> Dict[str, Any]:
        ...
        result = intellirecon_client.safe_post("api/intelligence/create-attack-chain", data)
    ...
    def intelligent_smart_scan(target: str, objective: str = "comprehensive", max_tools: int = 5) -> Dict[str, Any]:
        ...
        """
        logger.info(f"{IntelliReconColors.FIRE_RED}🚀 Starting intelligent smart scan for {target}{IntelliReconColors.RESET}")
        ...
        result = intellirecon_client.safe_post("api/intelligence/smart-scan", data)
        ...
        if result.get("success"):
            ...
            logger.info(f"{IntelliReconColors.SUCCESS}✅ Intelligent scan completed for {target}{IntelliReconColors.RESET}")
            logger.info(f"{IntelliReconColors.CYBER_ORANGE}📊 Execution Summary:{IntelliReconColors.RESET}")
            ...
            if successful_tools:
                logger.info(f"{IntelliReconColors.HIGHLIGHT_GREEN} Successful tools: {', '.join(successful_tools)} {IntelliReconColors.RESET}")

            # Log failed tools
            failed_tools = [t['tool'] for t in tools_executed if not t.get('success')]
            if failed_tools:
                logger.warning(f"{IntelliReconColors.HIGHLIGHT_RED} Failed tools: {', '.join(failed_tools)} {IntelliReconColors.RESET}")

            # Log vulnerabilities found
            if scan_results.get('total_vulnerabilities', 0) > 0:
                logger.warning(f"{IntelliReconColors.VULN_HIGH}🚨 {scan_results['total_vulnerabilities']} vulnerabilities detected!{IntelliReconColors.RESET}")
        else:
            logger.error(f"{IntelliReconColors.ERROR}❌ Intelligent scan failed for {target}: {result.get('error', 'Unknown error')}{IntelliReconColors.RESET}")
    ...
    def detect_technologies_ai(target: str) -> Dict[str, Any]:
        ...
        data = {"target": target}
        result = intellirecon_client.safe_post("api/intelligence/technology-detection", data)
    ...
    def ai_reconnaissance_workflow(target: str, depth: str = "standard") -> Dict[str, Any]:
        ...
        analysis_result = intellirecon_client.safe_post("api/intelligence/analyze-target", {"target": target})
        ...
        chain_result = intellirecon_client.safe_post("api/intelligence/create-attack-chain", {
            "target": target,
            "objective": objective
        })
        ...
        scan_result = intellirecon_client.safe_post("api/intelligence/smart-scan", {
            "target": target,
            "objective": objective,
    ...
    def bugbounty_reconnaissance_workflow(domain: str, scope: str = "", out_of_scope: str = "",
        ...
        result = intellirecon_client.safe_post("api/bugbounty/reconnaissance-workflow", data)
    ...
    def bugbounty_vulnerability_hunting(domain: str, priority_vulns: str = "rce,sqli,xss,idor,ssrf",
        ...
        result = intellirecon_client.safe_post("api/bugbounty/vulnerability-hunting-workflow", data)
    ...
    def bugbounty_business_logic_testing(domain: str, program_type: str = "web") -> Dict[str, Any]:
        ...
        result = intellirecon_client.safe_post("api/bugbounty/business-logic-workflow", data)
    ...
    def bugbounty_osint_gathering(domain: str) -> Dict[str, Any]:
        ...
        result = intellirecon_client.safe_post("api/bugbounty/osint-workflow", data)
    ...
    def bugbounty_file_upload_testing(target_url: str) -> Dict[str, Any]:
        ...
        result = intellirecon_client.safe_post("api/bugbounty/file-upload-testing", data)
    ...
    def bugbounty_comprehensive_assessment(domain: str, scope: str = "",
        ...
        result = intellirecon_client.safe_post("api/bugbounty/comprehensive-assessment", data)
    ...
    def http_framework_test(url: str, method: str = "GET", data: dict = {},
        ...
        if result.get("success"):
            ...
            if result.get("result", {}).get("vulnerabilities"):
                ...
                logger.info(f"{IntelliReconColors.HIGHLIGHT_RED} Found {vuln_count} potential vulnerabilities {IntelliReconColors.RESET}")
        else:
            logger.error(f"{IntelliReconColors.ERROR}❌ HTTP Framework {action} failed for {url}{IntelliReconColors.RESET}")
    ...
    def browser_agent_inspect(url: str, headless: bool = True, wait_time: int = 5,
        ...
        if result.get("success"):
            ...
            if action == "navigate" and result.get("result", {}).get("security_analysis"):
                ...
                if issues_count > 0:
                    logger.warning(f"{IntelliReconColors.HIGHLIGHT_YELLOW} Security Issues: {issues_count} | Score: {security_score}/100 {IntelliReconColors.RESET}")
                else:
                    logger.info(f"{IntelliReconColors.HIGHLIGHT_GREEN} No security issues found | Score: {security_score}/100 {IntelliReconColors.RESET}")
        else:
            logger.error(f"{IntelliReconColors.ERROR}❌ Browser Agent {action} failed for {url}{IntelliReconColors.RESET}")
    ...
    def burpsuite_alternative_scan(target: str, scan_type: str = "comprehensive",
        ...
        if result.get("success"):
            ...
            if result.get("result", {}).get("summary"):
                ...
                logger.info(f"{IntelliReconColors.HIGHLIGHT_BLUE} SCAN SUMMARY {IntelliReconColors.RESET}")
                ...
                for severity, count in vuln_breakdown.items():
                    if count > 0:
                        color = {
                                    'critical': IntelliReconColors.CRITICAL,
        'high': IntelliReconColors.FIRE_RED,
        'medium': IntelliReconColors.CYBER_ORANGE,
        'low': IntelliReconColors.YELLOW,
        'info': IntelliReconColors.INFO
    }.get(severity.lower(), IntelliReconColors.WHITE)

                        logger.info(f"  {color}{severity.upper()}: {count}{IntelliReconColors.RESET}")
        else:
            logger.error(f"{IntelliReconColors.ERROR}❌ Burp Suite Alternative scan failed for {target}{IntelliReconColors.RESET}")
    ...
    def error_handling_statistics() -> Dict[str, Any]:
        ...
        """
        logger.info(f"{IntelliReconColors.ELECTRIC_PURPLE}📊 Retrieving error handling statistics{IntelliReconColors.RESET}")
        result = intellirecon_client.safe_get("api/error-handling/statistics")
        ...
        if result.get("success"):
            ...
            logger.info(f"{IntelliReconColors.SUCCESS}✅ Error statistics retrieved{IntelliReconColors.RESET}")
            ...
            if error_counts:
                logger.info(f"{IntelliReconColors.HIGHLIGHT_BLUE} ERROR BREAKDOWN {IntelliReconColors.RESET}")
                for error_type, count in error_counts.items():
                                          logger.info(f"  {IntelliReconColors.FIRE_RED}{error_type}: {count}{IntelliReconColors.RESET}")
        else:
            logger.error(f"{IntelliReconColors.ERROR}❌ Failed to retrieve error statistics{IntelliReconColors.RESET}")
    ...
    def test_error_recovery(tool_name: str, error_type: str = "timeout",
        ...
        logger.info(f"{IntelliReconColors.RUBY}🧪 Testing error recovery for {tool_name} with {error_type}{IntelliReconColors.RESET}")
        result = intellirecon_client.safe_post("api/error-handling/test-recovery", data_payload)
        ...
        if result.get("success"):
            ...
            logger.info(f"{IntelliReconColors.SUCCESS}✅ Error recovery test completed{IntelliReconColors.RESET}")
            ...
            logger.error(f"{IntelliReconColors.ERROR}❌ Error recovery test failed{IntelliReconColors.RESET}")
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Run the IntelliRecon AI MCP Client")
    parser.add_argument("--server", type=str, default=DEFAULT_INTELLIRECON_SERVER,
                      help=f"IntelliRecon AI API server URL (default: {DEFAULT_INTELLIRECON_SERVER})")
...
def main():
    ...
    logger.info(f"🚀 Starting IntelliRecon AI MCP Client v6.0")
    logger.info(f"🔗 Connecting to: {args.server}")

    try:
        # Initialize the IntelliRecon AI client
        intellirecon_client = IntelliReconClient(args.server, args.timeout)

        # Check server health and log the result
        health = intellirecon_client.check_health()
        if "error" in health:
            logger.warning(f"⚠️  Unable to connect to IntelliRecon AI API server at {args.server}: {health['error']}")
            logger.warning("🚀 MCP server will start, but tool execution may fail")
        else:
            logger.info(f"🎯 Successfully connected to IntelliRecon AI API server at {args.server}")
            logger.info(f"🏥 Server health status: {health['status']}")
            logger.info(f"📊 Version: {health.get('version', 'unknown')}")
            if not health.get("all_essential_tools_available", False):
                logger.warning("⚠️  Not all essential tools are available on the IntelliRecon server")
                missing_tools = [tool for tool, available in health.get("tools_status", {}).items() if not available]
                if missing_tools:
                    logger.warning(f"❌ Missing tools: {', '.join(missing_tools[:5])}{'...' if len(missing_tools) > 5 else ''}")

        # Set up and run the MCP server
        mcp = setup_mcp_server(intellirecon_client)
        logger.info("🚀 Starting IntelliRecon AI MCP server")
